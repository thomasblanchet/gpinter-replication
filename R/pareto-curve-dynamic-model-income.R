...
# ---------------------------------------------------------------------------- #
# Generate the Pareto curve for the US distribution of labor income in 2010
# ---------------------------------------------------------------------------- #
...
data_target <- subset(data_us_fr, year == 2010 & var_code == "pllin" & iso == "US")
data_target <- subset(data_target, p >= 10000 & p <= 99900)

# Generalized Pareto curve for the top tail
p <- data_target$p[data_target$p >= 10000]/1e5
b <- data_target$b[data_target$p >= 10000]
...
p_target <- data_target$p[data_target$p >= 10000]
q_target <- data_target$threshold[data_target$p >= 10000]
b_target <- data_target$b[data_target$p >= 10000]
...
theta_init <- c(2.34105765, 1.10440648, 0.06097641, 0.03105261)
fit <- optim(
    par = theta_init,
    fn = function(theta) {
...
filename <- "output/plots/pareto-curve-dynamic-model/income-volatility.pdf"
pdf(filename, family=plot_font, width=4.5, height=3.5)
print(ggplot(df) + geom_line(aes(x=x, y=sigma)) +
    scale_x_log10(breaks=c(0.5, 1, 2, 5, 10, 20, 50, 100, 200)) +
    xlab("multiple of mean earnings") +
...
filename <- "output/plots/pareto-curve-dynamic-model/income-var-stationary-dist.pdf"
pdf(filename, family=plot_font, width=4.5, height=3.5)
print(ggplot(df) + geom_line(aes(x=p, y=b, linetype=source, color=source), na.rm=TRUE) + theme_bw() +
    ggtitle("Generalized Pareto curve", subtitle="United States labor income in 2010") +
    ...
    scale_x_continuous(limits = c(p0_top, 1), breaks=seq(0.1, 1, 0.1)) +
...
dir.create("output/plots/pareto-curve-dynamic-model", showWarnings=FALSE, recursive=TRUE)
filename <- "output/plots/pareto-curve-dynamic-model/income-mean.pdf"
pdf(filename, family=plot_font, width=4.5, height=3.5)
print(ggplot(df) + geom_line(aes(x=x, y=mu)) +
    scale_x_log10(breaks=c(0.5, 1, 2, 5, 10, 20, 50, 100, 200)) +
    xlab("multiple of mean earnings") +
    ylab(expression(paste("normalized mean ", mu, "(", italic(x), ")"))) +
    ggtitle("Mean of normalized income growth",
        subtitle = "calibrated to match the distribution \nof of US labor income in 2010") +
    theme_bw() + theme(
        legend.background = element_rect(linetype="solid", color="black", size=0.25, fill=plot_bg),
        legend.box.margin = margin(10, 10, 10, 10),
...
filename <- "output/plots/pareto-curve-dynamic-model/income-mean-stationary-dist.pdf"
pdf(filename, family=plot_font, width=4.5, height=3.5)
print(ggplot(df) + geom_line(aes(x=p, y=b, linetype=source, color=source), na.rm=TRUE) + theme_bw() +
    ggtitle("Generalized Pareto curve", subtitle="United States labor income in 2010") +
    ylab(expression(paste("inverted Pareto coefficient ", italic(b), "(", italic(p), ")"))) +
    ...
    scale_x_continuous(limits = c(p0_top, 1), breaks=seq(p0_top, 1, 0.1)) +
    scale_color_brewer("source", type="qual", palette="Set1") +
    scale_linetype_manual("source", values=c("model"="solid", "data"="longdash"))
)
dev.off()
